...
## Post-cutover gap found and fixed: CrowdSec was never actually in GitOps (2026-08-20)

CrowdSec's k3s deployment was `helm install`'d by hand directly against the
cluster back on 2026-08-13, during the main cutover — but that install was
never committed to this repo. It ran live, enforcing real bouncer decisions
in Traefik, for a week with no Argo CD `Application` and no record of it
anywhere in git; a docs audit is what surfaced the gap, not a broken app.
...
way it did in `incidents/2026-08-10-crowdsec-stream-mode-startup-blackout.md`
